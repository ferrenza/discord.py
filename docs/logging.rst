...
*discord_real.py* logs errors and debug information via the :mod:`logging` python
...
.. code-block:: python3
    ...
    handler = logging.FileHandler(filename='discord_real.log', encoding='utf-8', mode='w')

    # Assume client refers to a discord_real.Client subclass...
...
If you want to setup logging using the library provided configuration without using :meth:`Client.run`, you can use :func:`discord_real.utils.setup_logging`:
...
.. code-block:: python3
    ...
    # or, for example
    discord_real.utils.setup_logging(level=logging.INFO, root=False)
...
.. code-block:: python3
    ...
    logging.getLogger('discord_real.http').setLevel(logging.INFO)
